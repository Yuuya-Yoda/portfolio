...
# クラス
class Graph:
    def __init__(self, x, y, marker, color, label, type):
        self.x = x
        self.y = y
        self.marker = marker
        self.color = color
...
def main(graph):
    ...
    plt.title(MAIN_TITLE_NAME)    # グラフのタイトル
    plt.xlabel(X_TITLE_NAME)                # x軸のタイトル
    plt.ylabel(Y_TITLE_NAME)                     # y軸のタイトル
    ...
    for el in graph:
        if el.type == "plot":
            plt.plot(el.x, el.y, marker=el.marker, color=el.color, label=el.label)      # 折れ線
        elif el.type == "scatter":
            plt.scatter(el.x, el.y, color=el.color, label=el.label)
        else:
            raise Exception
    plt.legend(loc=LOC)  # 凡例
    if GRID:
        plt.grid()
    plt.show()

graph = []
graph.append(Graph(X1_LIST, Y1_LIST, MARKER1, COLOR1, LABEL1, GRAPH_TYPE1))
graph.append(Graph(X2_LIST, Y2_LIST, MARKER2, COLOR2, LABEL2, GRAPH_TYPE2))
graph.append(Graph(X3_LIST, Y3_LIST, MARKER3, COLOR3, LABEL3, GRAPH_TYPE3))
